...
useEffect(()=> {
    if(userSelected && show){
        console.log("usaurio"+userSelected.membership)
    setDataid(userSelected.id)
        setDataName(userSelected.name || "");
        setDataEmail(userSelected.email || "");
        setDataMembership(userSelected.membership || "")
        setDataDatePay(formatDate(userSelected.date_pay))
    }
},[userSelected, show]);
...
setDataMembership("");
    ...
    const submitForm = async () => {
        if(
            ...
            const response = await apiService.update(`/useraccount/update/${dataid}`, form)
            if(response.id){
                toast.success(`El usuario ${dataName} se actualizo correctamente`
                )
                resetForm();
                onUserEdited()
                onClose();
